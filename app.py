...
def main():
    parser = argparse.ArgumentParser(description="GitHub PR Comment Workflow Tool")
    parser.add_argument("--repo", required=True,
                       help="Repository in format owner/repo")
    parser.add_argument("--pr", required=True, type=int, help="PR number to fetch comments from")
    parser.add_argument("--save", action="store_true", 
                       help="Save formatted comments to a file")
    ...
    # If no PR number provided, show bootstrap prompt
    ...
    pr_data = workflow.fetch_pr_comments(args.pr_number)
    ...
    if args.reply:
        ...
        if not comment_info:
            print(f"Comment ID {args.reply} not found in PR #{args.pr_number}")
            sys.exit(1)
                    
        success = workflow.reply_to_comment(args.pr_number, args.reply, args.reply_body, comment_info['type'])
    ...
    if args.search:
        print(f"\nSearching for comments containing: '{args.search}'")
